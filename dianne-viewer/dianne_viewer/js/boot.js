...
overlayControls.innerHTML = [
  ...
  '  <span>1st</span>',
  '  <input id="iv-primary-opacity" type="range" min="0" max="1" step="0.01" value="1" style="width:35px;">',
  '</span>',
  ...
  '  <span>2nd</span>',
  '  <input id="iv-secondary-opacity" type="range" min="0" max="1" step="0.01" value="1" style="width:35px;">',
  '</span>',
...
document.addEventListener('keydown', e => {
  const tool = toolbar.getActiveTool();
  if (['annot_polygon', 'annot_wand', 'annot_lasso', 'annot_drag', 'ruler'].includes(tool)) annotationsCanvas.onKeyDown(e);
});
